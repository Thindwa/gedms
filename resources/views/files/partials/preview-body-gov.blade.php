...
<div class="gov-drive-preview-header flex items-center justify-between shrink-0 border-b border-slate-700/50">
    <span class="font-semibold text-white/95 truncate pr-2 flex items-center gap-2">
        <form :action="selectedFile ? '/files/' + selectedFile.id + '/favorite' : '#'" method="POST" class="inline shrink-0" x-show="selectedFile">
            @csrf
    ...
    <div class="flex items-center gap-2 shrink-0">
        ...
        <a :href="selectedFile ? '/files/' + selectedFile.id + '/preview' : '#'" target="_blank" rel="noopener" class="text-sm text-white/80 hover:text-white transition-colors">Open</a>
        <a :href="selectedFile ? '/files/' + selectedFile.id + '/download' : '#'" class="text-sm text-white/80 hover:text-white transition-colors">Download</a>
        <template x-if="selectedFile && selectedFile.isLockedByMe">
            <form :action="'/files/' + selectedFile.id + '/unlock'" method="POST" class="inline" @click.stop>
                @csrf
        ...
        <template x-if="selectedFile && !selectedFile.isLockedByMe && !selectedFile.locked_by">
            <form :action="'/files/' + selectedFile.id + '/lock'" method="POST" class="inline" @click.stop>
                @csrf
...
<div class="flex-1 flex min-h-0 overflow-hidden" x-show="selectedFile">
    <template x-if="selectedFile">
        ...
        <div class="flex-1 flex flex-row min-h-0 min-w-0 w-full">
            ...
            <div class="flex-1 min-w-0 min-h-0 flex flex-col bg-slate-100 overflow-auto">
                ...
                <div class="flex-1 min-h-0 min-w-0 flex flex-col p-4">
                    <div class="flex-1 min-h-0 min-w-0 rounded-lg bg-white shadow border border-slate-200 overflow-hidden" style="min-height: 400px;">
                        <iframe :src="'/files/' + selectedFile.id + '/preview'" class="w-full h-full border-0 block" style="width: 100%; height: 100%; min-height: 400px;" title="PDF Preview"></iframe>
                    </div>
                </div>
                </template>
                ...
                <div class="flex-1 min-h-0 min-w-0 flex items-center justify-center p-4 overflow-auto" x-data="{ imgError: false }" x-effect="imgError = false">
                    <div class="rounded-lg bg-white border border-slate-200 shadow-sm p-4">
                        <img x-show="!imgError" :src="'/files/' + selectedFile.id + '/preview?v=' + (selectedFile.version || 1)"
                             :alt="selectedFile.name"
                             class="max-w-full max-h-[70vh] w-auto h-auto object-contain"
                             loading="eager"
                             @@error="imgError = true">
                        <p x-show="imgError" class="text-sm text-amber-600 text-center py-4">Preview could not be loaded. <a :href="'/files/' + selectedFile.id + '/download'" class="text-indigo-600 hover:underline">Download</a></p>
                    </div>
                </div>
                </template>
                ...
                <div class="flex-1 flex flex-col items-center justify-center p-4 overflow-auto">
                    <div class="rounded-lg bg-white border border-slate-200 shadow-sm p-6 max-w-md w-full">
                        <audio :src="'/files/' + selectedFile.id + '/preview'" controls class="w-full"></audio>
                ...
                <div class="flex-1 flex flex-col items-center justify-center p-4 overflow-auto text-slate-500">
                    <div class="rounded-lg border border-slate-200 bg-white p-8 text-center shadow-sm">
                        ...
                        <a :href="'/files/' + selectedFile.id + '/download'" class="mt-2 inline-block text-sm text-indigo-600 hover:text-indigo-700 font-medium">Download file</a>
                    </div>
                </div>
                </template>
            </div>
            ...
            <div class="p-4" x-data="{ comments: [], loading: false }" x-init="$watch('selectedFile', async f => { if (f?.id) { loading = true; const r = await fetch('/files/' + f.id + '/comments'); comments = await r.json(); loading = false; } else { comments = []; } })">
