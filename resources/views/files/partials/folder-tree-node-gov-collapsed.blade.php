...
@endphp
<div class="pl-1" :class="!treeOpen && 'pl-0'">
    <a href="{{ route('files.index', ['space' => $spaceId ?? request()->get('space'), 'folder' => $node['id']]) }}"
       ...
       title="{{ $node['name'] }}">
        ...
        <span x-show="treeOpen" x-cloak class="truncate">{{ $node['name'] }}</span>
    </a>
    @if ($hasChildren && count($node['children']) > 0)
        <div x-show="treeOpen" x-cloak class="ml-3 mt-0.5 border-l border-slate-200 pl-2 space-y-0.5">
            @foreach ($node['children'] as $child)
                @include('files.partials.folder-tree-node-gov-collapsed', ['node' => $child, 'currentFolder' => $currentFolder, 'spaceId' => $spaceId ?? null])
            @endforeach
        </div>
    @endif
</div>
